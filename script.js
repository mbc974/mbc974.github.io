...
(function () {
  ...
  /* ---- Hero Spline : décor 3D premium en enhancement progressif ----
     Web Component <spline-viewer> chargé dynamiquement, desktop uniquement.
     Ne bloque pas le rendu, ne casse rien si le réseau/Spline échoue
     (le fallback glow CSS reste affiché). Décoratif & non focusable. */
  function initHeroSpline() {
    const host = document.querySelector('.hero-spline');
    if (!host) return;                                              // absent (ex. adhesion.html) → rien
    if (reduceMotion) return;                                       // mouvement réduit → pas de 3D
    if (!window.matchMedia('(min-width: 1024px)').matches) return;  // desktop uniquement
    if (host.dataset.splineInit) return;                            // une seule fois
    host.dataset.splineInit = '1';

    const SCRIPT_SRC = 'https://unpkg.com/@splinetool/viewer@1.12.97/build/spline-viewer.js';
    const SCENE_URL = 'https://prod.spline.design/NGGawNOIew1oR4dX/scene.splinecode';
    ...
    function mountViewer() {
      try {
        ...
        window.setTimeout(function () { host.classList.add('is-loaded'); }, 4500);
      } catch (e) { /* silencieux : le fallback CSS reste affiché */ }
    }
    ...
    if (window.customElements && window.customElements.get('spline-viewer')) {
      ...
      s.onerror = function () { /* échec réseau : fallback glow conservé, aucune erreur */ };
      document.head.appendChild(s);
    }
  }
  /* chargé après le rendu critique, sans bloquer le hero */
  if ('requestIdleCallback' in window) {
    window.requestIdleCallback(initHeroSpline, { timeout: 2500 });
  } else {
    window.setTimeout(initHeroSpline, 1200);
  }
})();
